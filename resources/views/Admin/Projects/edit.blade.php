        ...
        <div class="mb-3">
            <select name="type_id" id="type_id">
                ...
                @foreach ($types as $type)
                    <option @selected($type->id == old('type_id', $project->type_id)) value="{{$type->id}}">{{$type->slug}}
                    </option>
                @endforeach
            </select>
        </div>
        <div>
            ...
            <div class="d-flex gap-3 mt-3 mb-3">
                @foreach ($tecnologys as $tecnology)
                    <input @checked(in_array($tecnology->id, old('tecnologys', $project->tecnologies->pluck('id')->all()))) name="tecnologys[]"
